...
const AIChatBubble = observer(() => {
  ...
  const { userStore, dealsStore, contactsStore, messagesStore, calendarStore, dashboardStore } = rootStore
  ...
  // Ensure stores are initialized when chat opens
  useEffect(() => {
    if (isOpen && userStore.isAuthenticated) {
  ...
  }, [isOpen, location.pathname, userStore.isAuthenticated, dealsStore, contactsStore, messagesStore, calendarStore, dashboardStore])

  // Build context based on current route and stores
...
- AI Insights: ${(deal.aiInsights || []).map(i => `${i.type}: ${i.title} - ${i.description}`).join('; ') || 'None'}`
        ...
        // Deal not found yet, might be loading
...
- Action Items: ${actionItems.slice(0, 5).map(a => `${a.title} (${a.type})`).join('; ') || 'None'}`
  ...
  return (
    <>
      ...
      {/* Chat Window */}
      <div
        className={`fixed bottom-6 right-6 z-50 w-96 h-[600px] bg-card border border-border rounded-lg shadow-2xl flex flex-col transition-all duration-300 ${
        ...
        <div className="p-4 border-t border-border bg-sidebar">
          <div className="flex items-end space-x-2">
            <textarea
              ...
              rows={1}
              className="flex-1 min-h-[40px] max-h-32 p-2 bg-secondary border border-border rounded-lg text-foreground placeholder-muted-foreground resize-none focus:outline-none focus:ring-2 focus:ring-ring focus:border-transparent transition-colors overflow-y-auto"
              disabled={isLoading}
            />
            <button
