      ...
      <div className="flex items-center space-x-2">
        ...
        <div className={`relative flex-1 ${sizeClasses.container}`}>
          <div className="absolute left-3 top-1/2 transform -translate-y-1/2">
            <Search className={`${sizeClasses.icon} text-muted-foreground`} />
          </div>
          ...
          {showAI && !value && (
            <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
              <Brain className={`${sizeClasses.icon} text-primary`} />
            </div>
          )}
        </div>
        ...
        {showFilter && filters.length > 0 && (
          <div className="relative">
            <button
              ...
              className={`${sizeClasses.container} ${sizeClasses.button} bg-secondary border border-border rounded-lg text-foreground hover:bg-accent transition-colors flex items-center space-x-2`}
            >
              <Filter className={sizeClasses.icon} />
              <span>
                {selectedFilterOption?.label || 'Filter'}
                {selectedFilterOption?.count !== undefined && (
                  <span className="ml-1 text-muted-foreground">({selectedFilterOption.count})</span>
                )}
              </span>
            </button>
              ...
              <div className="absolute right-0 top-full mt-1 w-48 bg-card border border-border rounded-lg shadow-xl z-50">
                {filters.map((filter) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-accent transition-colors first:rounded-t-lg last:rounded-b-lg flex items-center justify-between ${
                      selectedFilter === filter.id ? 'text-primary bg-accent/50' : 'text-card-foreground'
                    }`}
                  >
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-xl z-50">
          <div className="p-3 border-b border-border">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm font-medium text-card-foreground">AI Suggestions</span>
            </div>
          </div>
